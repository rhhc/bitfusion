...
def get_bench_nn(bench_name, WRPN=False):
    ...
    with g.as_default():
        ...
        with g.name_scope('conv1'):  # prepare
            prev = conv(prev, filters=cin, kernel_size=1, stride=(
                1, 1, 1, 1), pad='SAME', c_dtype=FQDtype.FXP8, w_dtype=FQDtype.FXP8)

        if 'base' in bench_name:
            return g

        with g.name_scope('conv2'):  # profile
            prev = conv(prev, filters=cout, kernel_size=kernel_size,
                        stride=stride, pad=pad, group=group, c_dtype=fb, w_dtype=wb)
...
def save_list(lists=None, filename=None):
    assert len(
        lists) % 2 == 0, "list length should be even, but found %d" % len(lists)
...
def load_config():
    ...
    print("Layer configuration number (reduce redundant): %d" %
          length)  # 5626944 * 2
    ...
    return benchlist, interval


def get_benchlist(index=None):
    # TODO: test
    bench, interval = load_config()
    # 这都是什么含义呀
